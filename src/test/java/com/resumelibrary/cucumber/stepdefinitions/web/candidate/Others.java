...
public class Others extends Utility {
  ...
  public void iNavigateToPage(String url) {
    RemoteWebDriver tempDriver = (RemoteWebDriver) WebDriverProvider.threadLocalMap.get().get("webdriverObj");
    SessionId sessionID = tempDriver.getSessionId();

    StopWatch getUrlStopWatch = new StopWatch();
    getUrlStopWatch.start();
    logger.info("Web page url to load is : " + WebURLHelper.getWebUrl() + getURL(url));

    getDriverWithUrl(WebURLHelper.getWebUrl(), getURL(url));
    getUrlStopWatch.stop();
    System.out.println("Url Get time : " + String.valueOf(getUrlStopWatch.getTime() / 1000.00));
    logger.info("Url Get time : " + String.valueOf(getUrlStopWatch.getTime() / 1000.00) + " for sessionID : " + sessionID);

    StopWatch openUrlStopWatch = new StopWatch();
    openUrlStopWatch.start();
    webDriverWaitContainsUrl(getURL(url));
    openUrlStopWatch.stop();
    System.out.println("Url Open time : " + String.valueOf(getUrlStopWatch.getTime() / 1000.00));
    logger.info("Url Open time : " + String.valueOf(getUrlStopWatch.getTime() / 1000.00) + " for sessionID : " + sessionID);
  ...
  public void iShouldSeeTextPTagAndVerifyMessage(String ptagText, String textTobeVerified) {
    Assert.assertTrue(
      verifyTextMessagePTags(ptagText, textTobeVerified).toLowerCase().contains(textTobeVerified.toLowerCase()),
      "[---> " + textTobeVerified + " is not displayed <---]");
  }
